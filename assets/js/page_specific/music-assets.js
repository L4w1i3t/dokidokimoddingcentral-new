document.addEventListener("DOMContentLoaded", function () {
  ...
  function populateBgmGenreFilter() {
    ...
    if (
      currentSelection &&
      Array.from(bgmGenreFilter.options).some(
  ...
  function populateSfxCategoryFilter() {
    ...
    if (
      currentSelection &&
      Array.from(sfxCategoryFilter.options).some(
        (opt) => opt.value === currentSelection,
      )
    ) {
  ...
  function playMusic(item, playButton, visualizerBars) {
    ...
    if (item.file) {
      ...
      audioPlayer.play().catch((error) => {
        ...
        alert(
          "Could not play the audio file. It might be missing or unsupported by your browser.",
        );
        return;
      });
