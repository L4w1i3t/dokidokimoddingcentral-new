document.addEventListener("DOMContentLoaded", function () {
  ...
  function populateFilterDropdowns() {
    ...
    const savedBgCategory =
      bgCategoryFilter.value || currentBgCategory || "all";
    const savedCgCharacter =
      cgCharacterFilter.value || currentCgCharacter || "all";
    ...
    if (
      savedBgCategory &&
      Array.from(bgCategoryFilter.options).some(
        (opt) => opt.value === savedBgCategory,
      )
    ) {
    ...
    if (
      savedCgCharacter &&
      Array.from(cgCharacterFilter.options).some(
        (opt) => opt.value === savedCgCharacter,
      )
    ) {
  ...
  function renderBgGrid() {
    ...
    itemsToShow.forEach((item) => {
      ...
      previewBtn.addEventListener("click", () => {
        openPreviewModal(
          previewBtn.getAttribute("data-img"),
  ...
  function renderCgGrid() {
    ...
    itemsToShow.forEach((item) => {
      ...
      const imagePath = item.file
        ? `/data/art/${item.file}`
        : "/assets/images/cg/placeholder.jpg";
