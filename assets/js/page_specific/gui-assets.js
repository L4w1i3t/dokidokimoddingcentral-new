...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchGuiData() {
    try {
      ...
      guiGrid.innerHTML =
        '<div class="no-results"><p>Failed to load GUI elements. Please try again later.</p></div>';
    }
  }
  ...
  function filterGuiElements() {
    ...
    filteredGuiData = guiData.filter((item) => {
      ...
      const searchMatch =
        !searchTerm || nameMatch || authorMatch || keywordsMatch;
  ...
  function displayGuiElements(page) {
    ...
    if (pageData.length === 0) {
      guiGrid.innerHTML =
        '<div class="no-results"><p>No GUI elements found matching your criteria.</p></div>';
      setTimeout(() => {
  ...
  function updateGuiPagination() {
    totalGuiPages = Math.max(
      1,
      Math.ceil(filteredGuiData.length / ITEMS_PER_PAGE),
    );
