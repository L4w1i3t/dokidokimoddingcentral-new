document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderAddons() {
        ...
        addonsToShow.forEach(addon => {
            ...
            addonCardEl.setAttribute('data-compatibility', addon.compatibility.join(' '));
            
            // Format the compatibility string
            const compatibilityString = addon.compatibility.map(version => `Ren'Py ${version}.x`).join(', ');
    ...
    document.addEventListener('error', function(event) {
        if (event.target.tagName === 'IMG' && event.target.closest('.addon-image')) {
